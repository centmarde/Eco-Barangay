  ...
  <v-dialog :model-value="show" @update:model-value="$emit('update:show', $event)" max-width="600px" persistent>
    <v-card>
      <v-card-title class="text-h5 font-weight-bold">
        <v-icon left class="mr-2" color="primary">mdi-pencil-circle</v-icon>
        Edit Announcement
      </v-card-title>
...
import type { UpdateAnnouncementData, Announcement } from '@/stores/announcementsData'
...
interface Emits {
  ...
  (e: 'submit', data: UpdateAnnouncementData): void
  (e: 'cancel'): void
}
...
// Local form state
const localForm = ref<UpdateAnnouncementData>({
  title: '',
  description: '',
  image: ''
})
...
watch(
  ...
  (newAnnouncement) => {
    if (newAnnouncement) {
      ...
      imagePreview.value = newAnnouncement.image || ''
    }
  },
...
watch(
  ...
  (isShowing) => {
    if (isShowing) {
      ...
      // Load announcement data if available
      if (props.announcement) {
        localForm.value = {
          title: props.announcement.title,
          description: props.announcement.description,
          image: props.announcement.image || ''
        }
        imagePreview.value = props.announcement.image || ''
      }
    } else {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    let imageUrl = localForm.value.image || ''
    ...
    // Clean the form data
    const formData: UpdateAnnouncementData = {
